...
int __cstart(MAINFUNC* mainfunc)
{
  ...
  if (gcccrab->calltype == 5)
  {
    ...
    a=0; i=0;
    if (gcccrab->eplist->BeginArgs && (len = gcccrab->eplist->EndArgs-gcccrab->eplist->BeginArgs))
    {
      while (i<len)
      {
    ...
    if (!a)
    { /* Nothing from the EPLIST Args - use the command */
      for (i=0; i<8; i++)
      {
  ...
  else
  {
    ...
    for (a=i=0; i<8 && gcccrab->plist[gcccrab->argc][i]!=0xFF && gcccrab->plist[gcccrab->argc][i]!=' '; i++)
  ...
  rc = __CLVSTK(gcccrab->dynamicstack, mainfunc, gcccrab->argc, gcccrab->argv);
