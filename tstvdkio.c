...
static void fopen_t() {
  ...
  ASSERTNOTNULLP("fopen(TEMP FILE A V 512,r)", f=fopen("TEMP FILE A V 512","r"), f, if (f) fclose(f));
  ASSERTNOTNULLP("fopen(TEMP FILE A,r)", f=fopen("TEMP FILE A","r"), f, if (f) fclose(f));
  ASSERTNOTNULLP("fopen(TEMP FILE,r)", f=fopen("TEMP FILE","r"), f, if (f) fclose(f));
  ASSERTNULL("fopen(NOT MEMO A,r)", f=fopen("NOT MEMO A","r"), f, if (f) fclose(f));
  ASSERTNOTNULLP("fopen(NEW FILE A V 512,w)", f=fopen("NEW FILE A V 512","w"), f, if (f) fclose(f));
  ASSERTNOTNULLP("fopen(TEMP FILE A 512,a)", f=fopen("TEMP FILE A V 512","a"), f, if (f) fclose(f));
  ASSERTNULL("fopen(NOT FILE A V 512,a)", f=fopen("NOT FILE A V 512","a"), f, if (f) fclose(f));
  ASSERTNOTNULLP("fopen(TEMP FILE A,a)", f=fopen("TEMP FILE A","a"), f, if (f) fclose(f));

  /* Different Access types */
  ASSERTNOTNULLP("fopen(TEMP FILE,r+)", f=fopen("TEMP FILE","r+"), f, if (f) fclose(f));
  ASSERTNOTNULLP("fopen(TEMP FILE,rb+)", f=fopen("TEMP FILE","rb+"), f, if (f) fclose(f));
  ASSERTNOTNULLP("fopen(TEMP FILE,rb)", f=fopen("TEMP FILE","rb"), f, if (f) fclose(f));
  ASSERTNOTNULLP("fopen(NEW FILE A V 512,w+)", f=fopen("NEW FILE A V 512","w+"), f, if (f) fclose(f));
  ASSERTNOTNULLP("fopen(NEW FILE A V 512,w+b)", f=fopen("NEW FILE A V 512","w+b"), f, if (f) fclose(f));
  ASSERTNOTNULLP("fopen(NEW FILE A V 512,wb)", f=fopen("NEW FILE A V 512","wb"), f, if (f) fclose(f));
  ASSERTNOTNULLP("fopen(TEMP FILE A V 512,a+)", f=fopen("TEMP FILE A V 512","a+"), f, if (f) fclose(f));
  ASSERTNOTNULLP("fopen(TEMP FILE A V 512,a+b)", f=fopen("TEMP FILE A V 512","a+b"), f, if (f) fclose(f));
  ASSERTNOTNULLP("fopen(TEMP FILE A V 512,ab)", f=fopen("TEMP FILE A V 512","ab"), f, if (f) fclose(f));

  /* Bad Access */
  ASSERTNULL("fopen(TEMP FILE A V 512,x)", f=fopen("TEMP FILE A V 512","x"), f, if (f) fclose(f));
  ASSERTNULL("fopen(TEMP FILE A V 512,+)", f=fopen("TEMP FILE A V 512","+"), f, if (f) fclose(f));
  ASSERTNULL("fopen(TEMP FILE A V 512,b)", f=fopen("TEMP FILE A V 512","b"), f, if (f) fclose(f));
  ASSERTNULL("fopen(TEMP FILE A V 512,)", f=fopen("TEMP FILE A V 512",""), f, if (f) fclose(f));
  ASSERTNULL("fopen(TEMP FILE A V 512,r+bx)", f=fopen("TEMP FILE A V 512","r+bx"), f, if (f) fclose(f));

  /* Finally Check if write deletes */
  ASSERTNOTNULLP("fopen(TEMP FILE A,w)", f=fopen("TEMP FILE A","w"), f, if (f) fclose(f));
  ASSERTNULL("fopen(TEMP FILE,r)", f=fopen("TEMP FILE","r"), f, if (f) fclose(f));
}
...
static void fclose_t() {
  FILE* f;
  SUB_STRT("fclose()");
  ASSERTNOTNULLP("fopen(TEMP FILE A V 80,w)", f=fopen("TEMP FILE A V 80","w"), f, );
...
static void fflush_t() {
  ...
  SUB_STRT("fflush()");
  ASSERTNOTNULLP("fopen(NEW FILE A V 80,w)", test=fopen("NEW FILE A V 80","w"), test, );
  ASSERTNOTEOFP("fputc()", ,FPUTC_WR(TEST_SHORT_STRING,test), );
  ASSERTZEROP("fflush()", , fflush(test), );
  ASSERTZEROP("fclose()", , fclose(test), );
  ASSERTNOTNULLP("fopen(NEW FILE,r)", reader = fopen("NEW FILE","r"), reader, );
...
static void fgetc_t() {
  ...
  SUB_STRT("fgetc() - text");
  ASSERTNOTNULLP("fopen(TEST FILE A V,w)", out=fopen("TEST FILE A V","w"), out, );
...
static void ungetc_t() {
  ...
  ASSERTNOTEOFP("fputs()", , fputs(TEST_SHORT_STRING, test), );
  ASSERTZEROP("fclose()", , fclose(test), );
  ASSERTNOTNULLP("fopen(TEMP FILE A,rb)", test=fopen("TEMP FILE A","rb"), test, );
  ASSERTNOTEOF("ungetc(x)", , ungetc('x',test), );
  ASSERTEOF("ungetc(y)", , ungetc('y',test), );
...
static void fputc_t() {
  ...
  SUB_STRT("fputc() - text");
  ASSERTNOTNULLP("fopen(TEMP FILE A V 110,w)", test=fopen("TEMP FILE A V 110","w"), test, );
  FPUTC_T(test);
  ASSERTZEROP("fclose()", , fclose(test), );
  ASSERTNOTNULLP("fopen(TEMP FILE A,r)", test=fopen("TEMP FILE A","r"), test, );
  FGETS_T(test,110);
  ...
  ASSERTNOTNULLP("fopen(TEMP FILE A V 110,wb)", test=fopen("TEMP FILE A V 110","wb"), test, );
  buf2[0]=0;
  for (i=0; i<20; i++) {
...
static void fwriteread_t() {
  ...
  ASSERTNOTNULLP("fopen(TEMP FILE A V 105,wb)", test=fopen("TEMP FILE A V 105","wb"), test, );
  ...
  ASSERTNOTNULLP("fopen(TEMP FILE A V 105,w)", test=fopen("TEMP FILE A V 105","w"), test, );
...
static void fgetsetpos_t() {
  ...
  buffer2[count * strlen(item)] = 0;
  for (i=0; i<count; i++) strcat(buffer,item);
  ASSERTNOTNULLP("fopen(TEMP FILE A V,wb+)", test=fopen("TEMP FILE A V","wb+"), test, );
  ...
  ASSERTZERO("strcmp()", , strcmp(buffer,buffer2), );
  ASSERTZEROP("fclose()", , fclose(test), );
  ASSERTNOTNULLP("fopen(TEMP FILE A V 80,wb+)", test=fopen("TEMP FILE A V 80","wb+"), test, );
  ASSERTNOTEOFP("fputs()", , fputs("PREFIXABCDEFGHIJKLMNOPQRSTUVWXYZPREFIXABCDEFGHIJKLMNOPQRSTUVWXYZPREFIXABCDEFGHIJKLMNOPQRSTUVWXYZPREFIXABCDEFGHIJKLMNOPQRSTUVWXYZ",test),);
  ASSERTZERO("fgetpos()", , fgetpos(test, &position) != 0, );
  ASSERTNOTZERO("fwrite()", , fwrite(buffer, strlen(item), count, test)==count, );
  ASSERTZERO("fsetpos()", , fsetpos(test, &position) != 0, );
  ASSERTNOTZERO("fread()", ,fread(buffer2, strlen(item), count, test)==count, );
...
static void raccess_t() {
  ...
  for (i=0; i<500; i++) {
    r = (rand() % 99) + 1;
    rc = fsetrec(test,r);
    if (!rc) {TST_ERR("fsetrec error");break;}
  ...
  for (i=1; i<100; i++) writeRecordBin(test, i, 80, "-");
  ASSERTZEROP("fclose()", , fclose(test), );
  ASSERTNOTNULLP("fopen(TEMP FILE A V 80,rb)", test=fopen("TEMP FILE A V 80","rb"), test, );
  ...
  for (i=1; i<100; i++) writeRecordText(test, i, 80, "-");
  ASSERTZEROP("fclose()", , fclose(test), );
  ASSERTNOTNULLP("fopen(TEMP FILE A V 80,r)", test=fopen("TEMP FILE A V 80","r"), test, );
  ...
  /* Delete */
  fopen("TEMP FILE A V 80","w"); fclose(test);
}
...
static void nextrecLen_t() {
  ...
  ASSERTNOTNULLP("fopen(TEST FILE A,rb)", test=fopen("TEST FILE A","rb"), test, );
  ASSERTNOTZERO("nextrecLen()", , nextrecLen(test)==strlen(TEST_SHORT_STRING), );
  ASSERTNOTEOF("fgetc()", , fgetc(test), );
  ASSERTNOTZERO("nextrecLen()", , nextrecLen(test)==strlen(TEST_SHORT_STRING)-1, );
  ASSERTNOTNULL("fgets()", , fgets(buf, 300, test), );
  ASSERTNOTZERO("nextrecLen()", , nextrecLen(test)==strlen(TEST_SHORT_STRING), );
  ASSERTZEROP("fclose()", , fclose(test), );
}
